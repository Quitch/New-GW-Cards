...
This file provides guidance to Claude Code (claude.ai/code) and to human contributors
when working with code in this repository.

**Authoring vs maintaining.** This file governs changes to the template itself.
`CLAUDE.md` governs the other job — writing an actual card for a user against their PA
install — and travels with the repo when they copy it into `client_mods`. Read that one
when the user is authoring a card; keep the two in step when GWO's card contract or op
behaviour changes.

The repo root is also the mod root, so the release ZIP _is_ the folder that gets dropped
into `client_mods`. `CLAUDE.md`, `README.md`, the lint tooling and `.prettierrc` all ship
in it, because the audience takes the ZIP rather than cloning and the checker only works
because it sits beside `ui/`. This file is the one doc `.gitattributes` still
`export-ignore`s: it is about editing the template, not about writing a card.
...
The point of the repo is that an author copies the whole repo (normally the release ZIP)
into their PA `client_mods/` folder, renames it, and fills in the blanks. The following are
...
whole folder is what gets copied out, carrying `CLAUDE.md` (the authoring guide, above)
and the lint tooling with it. The rest of the root (`package.json`, `eslint.config.mjs`,
`.prettierrc`, `README.md`, `LICENSE`, this file) is invisible to PA, which only reads
`modinfo.json` and the files its `scenes` block names. Two distinct trees:
...
- Shipped game code targets PA's embedded **Chrome 40**. `ecmaVersion: 6` is a parser
  ...
  applied to `**/ui/**/*.js` (the tree is at `ui/**` beside the config, so the `**/`
  prefix is insurance against an author nesting their renamed copy inside another
  folder), with the post-ES5 features Chrome 40 genuinely shipped
