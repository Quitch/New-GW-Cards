...
function addDecks() {
  try {
    ...
    // }
    // model.gwoDecks.push({
    //   // A unique ID for the deck.  The war save remembers it, so changing
    //   // it later disconnects old wars from the deck.
    //   id: "YOUR-DECK-ID",
    //   // The name shown in the Techs picker and on the war panel.  The game
    //   // already labels these places "deck", so do NOT put the word Deck in
    //   // the name itself.
    //   name: "!LOC:YOUR NAME HERE",
    //   // OPTIONAL: one line describing the deck, added to the Techs
    //   // tooltip.  Remove the line if you do not want one.
    //   tooltip: "!LOC:ONE LINE DESCRIBING THE DECK.",
    //   // OPTIONAL: the IDs of other decks to include, cards and all.
    //   // "Basic" is the base game's deck and "Expanded" is the full
    //   // Galactic War Overhaul deck (which already contains Basic).  You
    //   // can also include another mod's deck: list that mod in
    //   // `dependencies` in modinfo.json AND give your mod a HIGHER
    //   // `priority` number than it, so it loads first.  Remove the line
    //   // for a standalone deck.
    //   include: ["Basic"],
    //   // OPTIONAL: individual card IDs.  Your own cards, or any stock
    //   // card - for example "gwc_minion" or "gwaio_upgrade_ant" - so you
    //   // can cherry-pick cards without including a whole deck.  A card
    //   // appears in the deck once however many times these lists name it.
