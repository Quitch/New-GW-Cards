...
- Shipped game code targets PA's embedded **Chrome 40**. `eslint.config.mjs` pins
  `ecmaVersion: 6` with only partial ES6 — stay ES5-ish; don't reach for modern JS the
  runtime lacks. Known engine globals are declared in the config
  (`api`, `model`, `_`, `requireGW`, `ko`, plus browser/jquery/amd).
- ESLint flat config, `js/recommended` + `curly: ["error", "all"]`, Prettier config
  ...
  `npm run lint:js`. `package.json` pins the four lint deps (`eslint`, `@eslint/js`,
  `eslint-config-prettier`, `globals`) with caret ranges and sets `"type": "module"`
  so Node loads the ESM `eslint.config.mjs` — that config file is the only `.js` Node
  itself executes; the shipped game code is only ever parsed by ESLint (as
  `sourceType: "script"`), never run under Node.
