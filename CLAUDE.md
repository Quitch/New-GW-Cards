...
| Race and add-on unit IDs (`gwoUnit.legion.shank`)       | `<GWO>` → `ui/mods/com.pa.quitch.gwaioverhaul/race/*.js` and `addon/*.js`, the `units` block. Keys are generated and change with race-mod updates                          |
