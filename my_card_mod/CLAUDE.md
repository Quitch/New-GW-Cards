...
user** from the New-GW-Cards template. It travels with this folder, so it applies both in
the template repo and in the copy the author renames into their `client_mods`.

`README.md` in the template repo is the author-facing guide, written in plain English for
...
Work in the user's copy. Inside the template repo itself, `my_card_mod/` is the pristine
skeleton — do not fill its placeholders in.
...
`eslint.config.mjs` in the template repo is the authoritative answer to "may I use X?":
it forbids everything after ES5 and then switches back on, one at a time, the features
Chrome 40 genuinely shipped. Anything not on that list is not allowed. To check a card,
put the mod folder inside the extracted template and run `npm install` once, then
`npm run lint:js`.
